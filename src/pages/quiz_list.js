...
const QuizList = () => {
    ...
    const openEditQuiz = (quiz) => {
        navigate("/admin/update-quiz-details", { state: { "quiz": quiz } });
    };
            ...
            <div>
                ...
                <section className="bg-gray-50 p-3 sm:p-5 antialiased mt-10">
                    <div className="flex flex-row md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                        <h1 className="text-3xl font-medium">Quiz List</h1>
                        <Link to="/admin/add-quiz" type="button" className="flex items-center justify-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 focus:outline-none">
                            Create New
                        </Link>
                    </div>
                    {loading ? (
                                        ...
                                        <tbody>
                                            {quizListResult.map((quiz, index) => (
                                                <tr key={quiz._id} className="border-b relative">
                                                    <th scope="row" className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap max-w-[8rem] truncate">{quiz._id}</th>
