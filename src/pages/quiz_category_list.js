...
const QuizCategoryList = () => {
    ...
    const [deletingQuizCategory, setDeletingQuizCategory] = useState(null); // State to track the quiz category to delete
            ...
            <div>
                ...
                <section className="bg-gray-50 p-3 antialiased mt-10">
                    <div className="flex flex-row md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                        <h1 className="text-3xl font-medium">Quiz Categories</h1>
                        <Link to="/admin/add-quiz-category" type="button" className="flex items-center justify-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 focus:outline-none">
                            Create New
                        </Link>
                    </div>
                    {loading ? (
                    ...
                    {!loading ? (
                        <div className="mx-auto max-w-screen-xl px-4 px-12">
                            {/*<!-- Start coding here -->*/}
                            <div className="bg-white relative shadow-md rounded-lg">
                                ...
                                <div className="overflow-x-visible">
                                    <table className="w-full text-sm text-left text-gray-500">
                                        ...
                                        <tbody>
                                            {quizCategoryListResult.map((quizCategory, index) => (
                                                <tr key={quizCategory._id} className="border-b relative">
                                                    <th scope="row" className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap max-w-[10rem] truncate">{quizCategory._id}</th>
                                                    <td className="px-4 py-3 max-w-[10rem] truncate">{quizCategory.title}</td>
                                                    <td className="px-4 py-3 relative">
                                                        <button onClick={() => toggleRowDropdown(quizCategory._id)} id={`apple-imac-27-dropdown-button-${quizCategory._id}`} className="inline-flex items-center text-sm font-medium hover:bg-gray-100 text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none " type="button">
                                                            <svg className="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                                                <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
                                                            </svg>
                                                        </button>
                                                        {/* Drop down */}
                                                        <div ref={rowDropdownRef} id={`apple-imac-27-dropdown-${quizCategory._id}`} className={`z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow absolute right-0 ${activeRowDropdown === quizCategory._id ? 'block' : 'hidden'} ${index >= quizCategoryListResult.length - 2 ? '-top-28' : 'top-10'}`}>
                                                            <ul className="py-1 text-sm" aria-labelledby={`apple-imac-27-dropdown-button-${quizCategory._id}`}>
                                                                <li>
                                                                ...
                                                                <li>
                                                                    <button onClick={() => openQuizCategoryPreview(quizCategory)} className="flex w-full items-center py-2 px-4 hover:bg-gray-100 text-gray-700">
                                                                        <svg className="w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                                                ...
                                                                <li>
                                                                    <button onClick={() => handleDeleteClick(quizCategory)} type="button" data-modal-target="deleteModal" data-modal-toggle="deleteModal" className="flex w-full items-center py-2 px-4 hover:bg-gray-100 text-red-500">
                ...
                {/*<!-- Delete modal -->*/}
                {deletingQuizCategory ? (
                    <div id="deleteModal" tabIndex="-1" aria-hidden="true" className="flex flex-col justify-center items-center fixed top-0 left-0 w-full h-full z-50">
                        <div className="relative p-4 max-w-lg max-h-full">
                            {/*<!-- Modal content -->*/}
                            <div className="relative p-4 text-center bg-white rounded-lg shadow">
                                <button onClick={() => setDeletingQuizCategory(null)} type="button" className="text-gray-400 absolute top-2.5 right-2.5 bg-transparent hover:bg-gray-200 hover-text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-toggle="deleteModal">
                                ...
                                <svg className="text-gray-400 w-11 h-11 mb-3.5 mx-auto" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                    <path fillRule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447a1 1 0 001-1H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clipRule="evenodd" />
                                </svg>
                                <p className="mb-4 text-gray-500">Are you sure you want to delete this item?</p>
                                <div className="flex justify-center items-center space-x-4">
                                    <button onClick={() => setDeletingQuizCategory(null)} data-modal-toggle="deleteModal" type="button" className="py-2 px-3 text-sm font-medium text-gray-500 bg-white rounded-lg border border-gray-200 hover-bg-gray-100 focus-ring-4 focus-outline-none focus-ring-primary-300 hover-text-gray-900 focus-z-10">No, cancel</button>
                                    <button onClick={confirmDelete} type="submit" className="py-2 px-3 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover-bg-red-700 focus-ring-4 focus-outline-none focus-ring-red-300">Yes, I'm sure</button>
                                </div>
                            </div>
                        </div>
                    </div>
